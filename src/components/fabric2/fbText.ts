...
fabric.FbText = fabric.util.createClass(fabric.Textbox, {
  type: "FbText",
  /**
  ...
  initialize: function (ele, options) {
    options || (options = {})
    this.callSuper("initialize", ele, options)
  },
  ...
  // 在这里重写了 _render 方法 希望给文字添加多重border
  _render: function (ctx) {
    // this.callSuper("_render", ctx)
    // const textLines = this.textLines
    // console.log(ctx, this._renderChar, getLineHeightFromCtx(ctx, textLines[0]), "ctx")
    // ctx.strokeStyle = "red"
    // ctx.strokeText(this.text, -this.width / 2, -10)
    // ctx.strokeStyle = this.stroke
    // ctx.lineWidth = 2
    // ctx.strokeText(this.text, -this.width / 2, 50)
    var path = this.path
